...
function print_ajde_customization_form($cutomization_pg_array, $nylon_option, $extra_tabs=''){
	...
	foreach($cutomization_pg_array as $cpa=>$cpav){								
		...
		$leftside .= "<li ".$ls_level_code."><a c_id='".$cpav['id']."' title='".$cpav['tab_name']."'>".$cpav['tab_name']."</a></li>";								
			...
			foreach($cpav['fields'] as $field){
				switch ($field['type']){
					...
					case 'text':
						...
						$rightside.= "<p>".$field['name']."</p><p><span class='nfe_f_width'><input type='text' name='".$field['id']."' value='".$this_value."'/></span></p>";
					break;
					case 'textarea':
						$rightside.= "<p>".$field['name']."</p><p><span class='nfe_f_width'><textarea name='".$field['id']."'>".$nylon_option[ $field['id']]."</textarea></span></p>";
					break;
					case 'font_size':
					...
					case 'radio':
						...
						foreach($field['options'] as $option=>$option_val){
							...
							$rightside.="<em><input type='radio' name='".$field['id']."' value='".$option."' "
							.  $checked_or_not  ."/> ".$option_val."</em>";
					...
					case 'dropdown':
						...
						foreach($field['options'] as $option=>$option_val){
							...
							.  ( ($option == $nylon_option[ $field['id'] ])? 'selected=\"selected\"':null)  ."/> ".$option_val."</option>";
						}						
						$rightside.= "</select></p>";						
					break;
					case 'checkboxes':
						...
						foreach($field['options'] as $option=>$option_val){
							$checked='';
							if(is_array($meta_arr)){$checked = (in_array($option, $meta_arr))?'checked':'';}
							
							$rightside.="<span><input type='checkbox' name='".$field['id']."[]' value='".$option."' ".$checked."/>".$option_val."</span>";							
						}						
						$rightside.= "</p>";						
					break;
					...
					case 'yesno':
						...
						$rightside.= "<p class='yesno_row'>".$legend_code."<a afterstatement='".$after_statement."' class='acus_yn_btn ".(($nylon_option[$field['id']]=='yes')?null:'btn_at_no')."'></a><input type='hidden' name='".$field['id']."' value='".(($nylon_option[$field['id']]=='yes')?'yes':'no')."'/><span>".$field['name']."</span></p>";
					break;
					case 'begin_afterstatement': 					
						$rightside.= "<div class='backender_yn_sec' id='".$field['id']."' style='display:".(($nylon_option[$field['id']]=='yes')?'block':'none')."'>";
					break;
