...
if ( ! function_exists( 'eventon_settings' ) ) {
	...
	function eventon_settings() {
		...
		do_action_ref_array('eventon_settings_tabs', array(&$evcal_tabs));
		...
		*/
		...
		if( isset($_POST['evcal_noncename']) && isset( $_POST ) ){
			if ( wp_verify_nonce( $_POST['evcal_noncename'], plugin_basename( __FILE__ ) ) ){
		...
		<?php					
			foreach($evcal_tabs as $nt=>$ntv){
				...
				if($nt=='evcal_5'){
					...
					$evo_notification = (!empty($licenses['eventon']['has_new_update']) && $licenses['eventon']['has_new_update'])? '<span class="evo_update_plugins"><span class="evo_update_count">1</span></span>':null;
				}else{$evo_notification='';}
...
switch ($focus_tab):
	...
	<form method="post" action=""><?php settings_fields('evcal_field_group'); 
		wp_nonce_field( plugin_basename( __FILE__ ), 'evcal_noncename' );
	?>
...
<?php	
	...
	require_once('includes/settings_licenses_tab.php');		
